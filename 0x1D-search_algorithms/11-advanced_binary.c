...
/**
 * helper2 - recursive helper function for the binary search
 * @array: input array
...
(*
 */
int helper2(int *array, size_t size, size_t first, size_t last, int value)
{
	size_t i;
	...
	if (value < array[(first + last) / 2])
		return (helper2(array, size, first, ((first + last) / 2), value));
	if (value > array[(first + last) / 2])
		return (helper2(array, size, (first + last) / 2 + 1, last, value));
	return (-1);
}
...
int advanced_binary(int *array, size_t size, int value)
{
	if (!array)
		return (-1);
	return (helper2(array, size, 0, size - 1, value));
}
